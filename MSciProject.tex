...
&\le \norm{\nabla m}_{L^2}\norm{\nabla v}_{L^2} + L\norm{m}_{L^2}\norm{\nabla v}_{L^2} + \lambda\norm{m}_{L^2}\norm{v}_{L^2}.
\end{align*}
By Cauchy--Schwarz and Assumption 3, there exists $c_1>0$ such that
\[
|a(m,v)| \le c_1\,\norm{m}_{H^1}\,\norm{v}_{H^1}.
\]
...
with fixed $m$, because the equation is non-linear so we resort to a fixed point method instead. The idea, is to create a map from the HJB equation with the right properties such that 
...
\end{proposition}
\begin{proof}
  Let $(m_k)$ converege to $m \in \Honez$. Denote $u=T[m]$, $u_k=T[m_k]$, $H=H(x,\nabla u)$, $H_k = H(x, \nabla u_k)$ and $f_k = f(x, m_k)$.
...
\begin{align*}
  ...
  &\le \tilde{C}\norm{u-u_k}_{L^2}\norm{\nabla(u-u_k)}_{L^2}
  \end{align*}
for which $\lambda >0$ being sufficiently large implies $u_k\to u$ i.e $T$ is continuous on $\Honez$.
...
\begin{proof}
  ...
  \end{align*}
  $m$ and $m_k$ are bounded in $\Honez$ so as $k$ gets large \[\int_\Omega|\nabla(m-m_k)|\,|m_k||\beta_k|\,dx \to 0.\] 
...
\begin{proof}
  ...
  We also have that 
  \[\norm{H(x,\,\nabla u_k)}_{\Ltwo} \le C(\norm{\nabla u_k}_{\Ltwo} + |\Omega|^\frac12)\] which is uniformly bounded by Assumption 2 and the boundedness of $(u_k)$ in $\Honez$. This means that both $f_k =f(x,m_k)$
...
We have shown that $A$ is continuous and compact in $\Honez$. The last step before we can apply Schauder's theorem is to prove 
...
By Schaefer's theorem (\Cref{thm:schaefer}), $A$ has a fixed point. Thus, a solution pair $(u,m) \in \Honez \times \Honez$ to the stationary Mean Field Game sytem exists.
\end{proof}
...
the image of $S$ is always bounded (\cref{rm:s-bounded}) independently of $m$.
...
\end{proof}
\begin{remark}
  This result has a very important implication. It tells us that any solution $m \in \Honez$ is still bounded by a constant despite $G(\cdot)$ being non-linear. 
\end{remark}
\begin{proposition}
...
A fixed point of $\tilde{A}$ means a solution to the new sytem of equations exists. 
