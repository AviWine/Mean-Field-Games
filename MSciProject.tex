...
In this paper we consider the Nash equilibrium problem characterised by a coupled pair of equations: a Hamilton--Jacobi--Bellman (HJB) equation and a Kolmogorov--Fokker--Planck (KFP) equation.
...
On the other hand, (KFP) gives the evolution of $m(\cdot)$ when every player satisfies the optimal solution $u$.
...
In this paper we analyse the coupled system \cref{eq:system} and investigate the existence of a weak solution pair $(u,m)\in\Honez\times\Honez$. Whether 
...
We know $w$ exists by Lax--Milgram \cref{thm:lax-milgram}, so $A$ is well defined.\footnote{LHS is bounded and coercive  and RHS is bounded.} Furthermore by regularity theory \footnote{See \cite[Section~6.3]{evans_pde}} we have 
\begin{align*}
  \norm{w}_{H^2(\Omega)}
  & \le C \norm{f(x,m)-H(x,\nabla u)}_{\Ltwo}\\
  & \le C \norm{f(x,m)}_{\Ltwo} + C\norm{H(x,\nabla u)}_{\Ltwo}\\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\le \int_\Omega |fu| + |H(x,\nabla u)u|\,dx, \qquad \text{ since } 0\le \mu \le 1\\
 ...
 Provided $\lambda>\frac{C^2}{2}$ there exist sufficiently small $\varepsilon> 0$ and sufficiently large $\delta < 1$ such that
  \[\lambda > \frac12(\varepsilon^2 + \frac{C^2}{\delta^2}) \text{ and } 1 > \delta^2\]
  hold simultaneously.
  Taking the difference of both sides and denoting the constant factor as $K \coloneqq \int_\Omega \delta^2 \,dx$, yields 
  \begin{align*}
      \alpha \int_\Omega |\nabla u|^2 + |u|^2\,dx
      &\le \frac{1}{2\varepsilon ^2}\int_\Omega f(x,m)^2 \,dx + K,
    \end{align*}
  where $\alpha$ is the minimum of both factors of $u^2\text{ and } \nabla u^2$. 
  Equivalently, 
\[\alpha \norm{u}_{\Honez} \le \frac{1}{2\varepsilon ^2} \norm{f(x,m)}_{\Ltwo}+ K.\]
Thus if $\lambda>\frac{C^2}{2}$, the set 
\[\{u\in \Honez: u=\mu A[u]\ \text{for some }0\le\mu\le 1\}\]
is bounded independently of $\mu$. By \cref{thm:schaefer}, $A$ has a fixed point and thus (HJB) has a solution.
\end{proof}
\begin{remark}
...
Having solved the two equations in \cref{eq:system} in isolation, we now look to solve them as a system. As stated above, the fixed point theorems 
...
\begin{definition}
\label{def:A-map}
Let $T$ and $S$ be as in definitions \cref{def:S-map} and \cref{def:T-map}. Define
...
\begin{lemma}
  \label{thm:A-cont}
  Let $A$ be defined as in \cref{def:A-map} and suppose that $\lambda > 0$ as it appears in \cref{eq:system} is sufficiently large.
...
Reapplying the boundedness argument from \cref{thm:existence_hjb} and noting that by it's convegence to $f, \, f_k$ is bounded, we get that $(u_k)$ is bounded in $\Honez$.
...
\begin{proof}
  ...
  $\lambda$ (see \cref{prop:kfp-coercive}), we get $m_k \to m$ i.e $S$ is continuous.
\end{proof}
...
Let A be as in \cref{def:A-map}. A is compact and continuous and has bounded fixed points (if they were to exist) by the above, \cref{thm:A-cont} and \cref{thm:A-comp}.
By Schaefer's theorem (\cref{thm:schaefer}), $A$ has a fixed point. Thus, a solution pair $(u,m) \in \Honez \times \Honez$ to the MFG sytem exists.
\end{proof}
...
the image of $S$ is always bounded (\cref{thm:lax-milgram}) independent of $m$.
An equaly valid construction for $A$ however, is the reverse composition 
\[A \coloneqq T\circ S.\] 

Here too the existence of a fixed point proves \cref{thm:main_result} yet crucially, recall that $T$ has no obvious bound. 
...
We obtain the weak form of (HJB) with $v = u$
  \begin{align*}
    ...
    &\le \int_\Omega |fu| + \mu\,|H(x,\frac{\nabla u}{\mu})u|\,dx \qquad \text{ since } 0\le \mu \le 1\\
...
\begin{proof}
  ...
  \end{align*}
  by the Lax-Milgram bound (\cref{thm:lax-milgram}). Thus, the set is bounded.
\end{proof}
...
\begin{proof}
  ...
  The set of fixed points is bounded by \cref{prop:f_bounded}. 
  Thus, there exists a fixed point by Schaefer's Theorem. 
\end{proof}

\section{Extension to Non-linear (KFP)}

The following is an analysis on a variation of \cref{eq:system} with the $G(\cdot)$ in question being $m$ dependent. 
...
\begin{proof}
  ...
  The only difference is in the thershold of $\lambda$ which we'll now derive. As in \cref{thm:existence_hjb} we want to show
  \[\{m\in \Honez: m=\mu A[m]\ \text{for some }0\le\mu\le 1\}\]
  is bounded.
  Substituting $w=m/\mu$ and taking $v=m$, we obtain the weak form
  \begin{align*}
  ...
  \begin{align*}
  \int_\Omega |\nabla m|^2 + \lambda |m|^2\,dx
  &\le \int_\Omega \frac12\delta ^2\,|\nabla m|^2 + \bigl(B+\frac12 \varepsilon^2 +\frac{L^2}{2\delta ^2}\bigr)\,|m|^2 \,dx  + K
...
As with the linear version of (KFP) we define a map $\tilde{S}$ that will now take the place of $S$ in \cref{def:S-map}.
\begin{definition}
\label{def:s_tilde}
...
\begin{proof}
  ...
  We also have that $\tilde{S}$ is bounded (\cref{thm:existence2}) and that $T$ is both compact (\cref{prop:t-comp}) and continuous (\cref{prop:t-cont}). $\tilde{A}$ is therefore
  compact.
  \paragraph{}
...
\newpage
\section*{Note to Noga (quick context + what to look for)}
\addcontentsline{toc}{section}{Note to Noga (quick context + what to look for)}

Hi Noga,

Thanks so much for taking a look at this draft. This is obvs my MSci project in analysis/PDEs, but I’m mainly sending it to you for a \emph{readability / exposition} critique rather than for detailed mathematical verification.

\paragraph{What the paper is trying to do (in one sentence).}
I’m proving \emph{existence of weak solutions} to a coupled pair of PDEs coming from mean field games: a Hamilton--Jacobi--Bellman (HJB) equation coupled to a Kolmogorov--Fokker--Planck (KFP) equation. The “game theory” motivation isn’t essential for your read---you can ignore the economics / control background if you like.

\paragraph{The PDE system you’ll see.}
The model system is (with $\lambda>0$ and a bounded domain $\Omega\subset\mathbb{R}^n$):
\begin{align*}
-\Delta u + H(x,\nabla u) + \lambda u &= f(x,m) \qquad &&\text{in }\Omega \quad\text{(HJB)},\\
-\Delta m - \operatorname{div}\!\bigl(m\,\tfrac{\partial H}{\partial p}(x,\nabla u)\bigr)+\lambda m &= G(x)
\qquad &&\text{in }\Omega \quad\text{(KFP)},
\end{align*}
with homogeneous Dirichlet boundary conditions
\[
u=0,\qquad m=0\qquad \text{on }\partial\Omega.
\]
Very informally: $u$ is a “value function” and $m$ is a “density”. The coupling is that the HJB contains $m$ through $f(x,m)$, and the KFP contains $u$ through the drift term involving $\tfrac{\partial H}{\partial p}(x,\nabla u)$.

\paragraph{What “weak solution” means here (high-level).}
Rather than solving these equations pointwise (which would require a lot of smoothness), I work in Sobolev spaces and interpret the equations in an \emph{integral} (variational) sense. 
This is (I'm pretty sure) one 20th century maths' workarounds for PDE problems that are too complicated to solve classically. It took ages for any of it to make sense to me and I'm not sure it does even now so you are forgiven if it doesn't click.
Concretely, when you see a “weak formulation”, it usually means:
\begin{itemize}
\item multiply the PDE by a test function $v$,
\item integrate over $\Omega$,
\item integrate by parts to move derivatives off the unknowns onto the test functions,
\item interpret all terms using $L^2$-type integrability.
\end{itemize}
You don’t need to check those derivations unless you want to (fat chance ay?) - I'll just state something like 'this gives the weak form' and give it; I mainly want to know if the narrative is clear.

\paragraph{The $L^2$ norm.}
For a function $g:\Omega\to\mathbb{R}$, the $L^2$ norm is
\[
\|g\|_{L^2(\Omega)} \coloneqq \left(\int_\Omega |g(x)|^2\,dx\right)^{1/2}.
\]
Intuitively, this measures the “energy size” of the function (square-integrability).

\paragraph{The $H^1$ norm.}
The Sobolev space $H^1(\Omega)$ consists of functions $g$ such that both $g$ and its weak (i.e. when integrated) first derivatives are in $L^2(\Omega)$.
The corresponding norm is
\[
\|g\|_{H^1(\Omega)} \coloneqq \left(\|g\|_{L^2(\Omega)}^2 + \|\nabla g\|_{L^2(\Omega)}^2\right)^{1/2}.
\]
So $H^1$ controls both the size of the function and the size of its gradient.

\paragraph{What $H^1_0(\Omega)$ is (intuition).}
$H^1_0(\Omega)$ is the space of $H^1$ functions (i.e have finite $H^1$ norms) that “vanish on the boundary” ($=0 \text{ on } \partial \Omega$ in the weak sense).
A clean intuition is:
\[
H^1_0(\Omega) = \overline{C_c^\infty(\Omega)}^{\,\|\cdot\|_{H^1}},
\]
i.e.\ it’s the closure (in the $H^1$ norm) of smooth functions compactly supported inside $\Omega$.  
So elements of $H^1_0(\Omega)$ can be approximated by smooth functions that are literally zero near $\partial\Omega$, which is why they encode the Dirichlet boundary condition $u=0$ (and similarly $m=0$).

\paragraph{What I’d love you to critique.}
If you have time, I’d really appreciate comments on:
\begin{itemize}
\item whether the “big picture” is easy to follow (what is being proved and why the steps are arranged that way),
\item whether definitions/theorems are introduced at the right time,
\item any places where notation feels introduced too abruptly,
\item whether the transitions between sections feel smooth,
\item general readability: sentence length, clarity, signposting, and whether anything feels repetitive or confusing.
\end{itemize}
Feel free to ignore technical details (constants, exact thresholds on $\lambda$, etc.) unless something looks clearly inconsistent.

Thanks again --- any feedback is hugely appreciated.
