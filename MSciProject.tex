...
Under suitable assumptions, we establish the existence of a weak solution pair in $H_0^1(\Omega)\times H_0^1(\Omega)$, and we extend the argument to a variant with a non-linear source term in the KFP equation.
\end{abstract}
...
\begin{enumerate}[label=\arabic*., leftmargin=*, itemsep=0.35em]
  \item $\Omega$ is bounded and $\partial\Omega$ is $C^1$.
  \item $H(x,\nabla u):\R^{n+1}\to\R$ is
  ...
  \end{itemize}
  \item The partial derivative $\Hp(x,p)$ is continuous and essentially bounded by some constant $L$.
...
Before starting our proof it is important to introduce the following inequalities, which will be used repeatedly and will lend themselves
to many proofs throughout this paper.
...
\end{proposition}
\begin{proof}
  Let $(m_k)_{k=1}^\infty\to m$ in $\Honez$. Denote $u=T[m]$, $u_k=T[m_k]$, $H=H(x,\nabla u)$, $H_k = H(x, \nabla u_k)$ and $f_k = f(x, m_k)$.
...
\[\alpha \norm{u_k}_{\Honez} \le \frac{1}{2\varepsilon ^2} \norm{f_k}_{\Ltwo}+ K.\]
 where $K, \,m $ and $\varepsilon$ are independent constants. By the fact that $f_k \to f$ we know that $\norm{f_k}_{\Ltwo}$ 
 is bounded and so therefore $(u_k)$ is bounded in $\Honez$.
Thus, ${\norm{u-u_k}_{L^2}\norm{f-f_k}_{L^2}\to 0}$. Therefore, in the limit of $k$, (1) becomes
\begin{align*}
  \norm{\nabla(u-u_k)}_{L^2}^2+\lambda\norm{u-u_k}_{L^2}^2
  &\le \tilde{C}\norm{u-u_k}_{L^2}\norm{\nabla(u-u_k)}_{L^2}
  \end{align*}
...
\end{proposition}
\begin{proof}
  Let $(u_k)_{k=1}^\infty\to u$ in $\Honez$. Denote $m=S[u]$, $m_k=S[u_k]$, $b = \Hp(x, \nabla u)$ and $b_k=\Hp(x,\nabla u_k)$.
  ...
  Define the correction vector field $\varepsilon_k \coloneqq b - b_k$. Note that since $b_k$ is continuous w.r.t. $u_k$ we have
  $\lim_{k \to \infty} \norm{\varepsilon_k}_{\Ltwo(\Omega)} = 0$. Then, 
  \begin{align*}
  \norm{\nabla(m-m_k)}_{L^2}^2+\lambda\norm{m-m_k}_{L^2}^2
...
\begin{proof}
  ...
  &\le \tilde{C}\norm{u-u_{k_j}}_{L^2}\norm{\nabla(u-u_{k_j})}_{L^2} + \norm{u-u_{k_j}}_{L^2}\norm{f-f_{k_j}}_{L^2}
  \end{align*}
  Because $f$ is $\Ltwo$-continuous (Assumption 5),  the continuity argument for $T$ applies with $m_{k_j}$.
...
By Schaefer's theorem, $A$ has a fixed point. We may conlclude our finding with the proof of \Cref{thm:main_result}.
...
By Schaefer's theorem (\Cref{thm:schaefer}), $A$ has a fixed point. Thus, a solution pair $(u,m) \in \Honez \times \Honez$ to the stationary MFG sytem exists.
\end{proof}
...
the image of $S$ is always bounded (\cref{rm:s-bounded}) independent of $m$.
...
recall that we have not shown the image of $T$ to have any natural bound in $\Honez$. 
We note that bounded sets under $S$ remain bounded and $A$ retains its continuity and compactness.
...
\[\alpha \norm{u}_{\Honez} \le \frac{1}{2\varepsilon ^2} \norm{f(x,m)}_{\Ltwo}+ K\]
with $\alpha, \, \varepsilon$ and $K$  independent constants.
...
\begin{proof}
  ...
  Let $(u_k)_{k=1}^\infty\to u$ in $\Honez$. Denote $m=\tilde{S}[u]$, $m_k=\tilde{S}[u_k]$, $b=\Hp(x, \nabla u)$, $b_k=\Hp(x,\nabla u_k)$ and $G_k = G(x,m_k)$.
...
For a fixed density $m$, the Hamilton--Jacobi--Bellman equation is solved to obtain $u \in H^1_0(\Omega)$, while for a fixed $u$ the Kolmogorov--Fokker--Planck equation determines the corresponding density $m$. 
These steps define two operators between the relevant function spaces. By composing these operators we obtain a map whose fixed points correspond to solution pairs $(u,m)$ of the coupled system.\\

A key feature of this construction is that one ordering of the composition allows the existence result to be obtained without imposing a growth 
condition on the coupling term $f$. The fixed point framework therefore provides a flexible mechanism for 
passing from solvability of the individual equations to the existence of a solution for the full mean field game system. We further extend this analysis by incorporating 
nonlinear source terms $G(x,m)$ in the KFP equation and show that, under suitable structural assumptions, the same argument yields existence of steady state solutions in this more general setting. \\
Finally, since $\Omega$ is bounded, continuity assumptions on the coupling terms may equivalently be formulated in $L^1$ or $L^2$ (since \Cref{thm:holder} makes the two conditions interchangeable up to a constant), 
illustrating the robustness of the functional analytic approach employed here.
