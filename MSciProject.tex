...
\begin{enumerate}[label=\arabic*., leftmargin=*, itemsep=0.35em]
  ...
  \end{itemize}
  \item The partial derivative $\Hp(x,p)$ is $L^2$ continuous and essentially bounded by some constant $L$.
...
\begin{proof}
  ...
  Define the correction vector field $\varepsilon_k \coloneqq b - b_k$. Note that since $b_k$ is $L^2$ continuous w.r.t. $u_k$ we have
  $\norm{\varepsilon_k}_{\Ltwo} \to 0$ (Assumption 3). Then, 
  \begin{align*}
  \norm{\nabla(m-m_k)}_{L^2}^2+\lambda\norm{m-m_k}_{L^2}^2
...
recall that we have not shown the image of $T$ to have any constant bound in $\Honez$. 
...
\begin{proof}
Suppose $m_1,m_2$ both solve the non-linear KFP equation. Taking the difference and setting $v=m_1-m_2$ gives
\begin{align*}
...
\begin{definition}
\label{def:s_tilde}
Define $\tilde{S}:\Honez\to\Honez$ by $\tilde{S}(u)=m$, where $m$ solves non-linear KFP for given $u$.
\end{definition}
