...
\begin{enumerate}[label=\arabic*., leftmargin=*, itemsep=0.35em]
  ...
  \begin{itemize}[leftmargin=*, itemsep=0.25em]
    ...
    \item bounded by $D\bigl(|m|+1\bigr)$ for a constant $D$, for all $x\in \Omega$ and $\nabla u\in\R^n$.
  \end{itemize}
...
\begin{proof}[Proof of \Cref{thm:A-cont}]
  $A$ is continuous by the fact that T is continuous (\Cref{prop:t-cont}) and S is continuous (\Cref{prop:s-cont}).
\end{proof}
...
that $v_i=\partial_i u$. Therefore, by a diagonal argument there exists a common subsequence still denoted $(u_k)$
\[
...
\begin{proof}
  ...
  By the growth condition on $f$ (Assumption 5) we have
  \begin{align*}
    \alpha \norm{u_k}_{\Honez}
  ...
  \end{align*}
  which is uniformly bounded by our choice of a bounded sequence $(m_k)$.
  We also have that 
  \[\norm{H(x,\,\nabla u_k)}_{\Ltwo} \le C(\norm{\nabla u_k}_{\Ltwo} + |\Omega|^\frac12)\] which is uniformly bounded by Assumption 2 and the boundedness of $(u_k)$. This means that both $f_k =f(x,m_k)$
...
\end{proof}
\begin{remark}
  Note that we do not actually make use of the fact that $(m_k)$
\end{remark}
...
\end{proof}
\begin{remark}
  Notice the similarity in the proofs of \Cref{prop:t-comp} and \Cref{prop:f_bounded}. They both illustrate that the image $T$ is bounded on a bounded domain. 
  They key difference when considering a fixed point of the map as a whole, is in the domains of $T$. In the first composition $T$ is acting on all $m \in \Honez$ whereas in the latter it acts on 
  the image of $S$. 
\end{remark}
